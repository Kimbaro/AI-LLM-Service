...
import com.kimbaro.www.ai_llm_module.config.init.ThreadPoolGroups
...
class Gemini_LLMController(private val threadPoolGroups: ThreadPoolGroups, private val applicationContext: ApplicationContext) {
    ...
    fun requestLLM(@RequestBody llmDTO: RequestLLMDto, session: WebSession): ResponseEntity<String> {
...
//        threadPoolGroups.enqueue(GeminiThreadData(session,llmDTO))   <<- 비동기로 처리해야하는 경우
        ...
        return obj.execute(GeminiThreadData(session,llmDTO))
    }
}